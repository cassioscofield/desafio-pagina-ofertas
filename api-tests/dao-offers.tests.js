...
describe('offersDao\n', function() {
    ...
    describe('retrieveOne', function() {
        ...
        it('should retrieve single offer with all options ordered by price', function(done) {
            ...
            offersDao.retrieveOne({id: '0', orderBy: 'price'}, function (err, data) {
                assert(err === null);
                assert(data.id === 0);
                assert(data.options.length === 2);
                assert(data.options[0].price <= data.options[1].price);
                done();
            });
